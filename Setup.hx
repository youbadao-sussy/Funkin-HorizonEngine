...
class Setup
{
  public static function main():Void
  {
    ...
    var i = 0;
    var quiet = "/dev/null";
    if (Sys.systemName() == "Windows") quiet = "nul";
    ...
    for (data in libs)
    {
      ...
      switch (data.type)
      {
        case "install", "haxelib": // for libraries only available in the haxe package manager
          ...
          if (!FileSystem.exists(".haxelib/" + data.name)
            || (FileSystem.exists(".haxelib/" + data.name + "/.current") ? File.getContent(".haxelib/" + data.name + "/.current") != data.version : true))
          {
        ...
        case "git": // for libraries that contain git repositories
          ...
          if (!FileSystem.exists(".haxelib/" + data.name)
            || (FileSystem.exists(".haxelib/" + data.name + "/.current") ? File.getContent(".haxelib/" + data.name + "/.current") != "git" : true))
          {
            var progress = Math.floor(i / libs.length * 1000) / 10;
