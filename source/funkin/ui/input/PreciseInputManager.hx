  ...
  public function initializeButtons(controls:Controls, gamepad:FlxGamepad):Void
  {
    ...
    for (noteDirection in DIRECTIONS)
    {
      ...
      for (button in buttons)
      {
        ...
        {
          buttonListEntry = [];
          _buttonList.set(gamepad.id, buttonListEntry);
        }
        buttonListEntry.push(button);
        ...
        {
          buttonListMapEntry = new Map<FlxGamepadInputID, FlxInput<FlxGamepadInputID>>();
          _buttonListMap.set(gamepad.id, buttonListMapEntry);
        }
        buttonListMapEntry.set(button, input);

        // ボタンリストディレクションエントリの存在を確認
        var buttonListDirEntry = _buttonListDir.get(gamepad.id);
        if (buttonListDirEntry == null)
        {
          buttonListDirEntry = new Map<FlxGamepadInputID, NoteDirection>();
          _buttonListDir.set(gamepad.id, buttonListDirEntry);
        }
        buttonListDirEntry.set(button, noteDirection);
      }
    }
